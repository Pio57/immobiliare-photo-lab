...
def build_request_js(model: str, image_expr: str, stats_expr: str) -> str:
    """Anthropic request for the AI diagnoser. `image_expr` is a JS expression for the
    image source block, `stats_expr` one for the stats object."""
...
return {{ json: {{ t0: Date.now(), model: {js_string(model)},
...
const t0 = $('{request_node}').first().json.t0;
...
"""
    nodes = [
        ...
        sticky("Tre versioni, alla cieca", "Il record fissa un ordine casuale (V1..V3) e va in experiments/runs. Il sito, che ha gia' ricevuto l'image_id, chiede il risultato al workflow Scelte finche' non e' pronto. L'agente sceglie (o tiene l'originale); il nome del workflow si scopre dopo. Una versione bocciata dal gate si vede ma non si puo' scegliere.", 3900, 180, 900, 300, 2),
