...
def test_prepare_carries_the_four_turns(room):
    import base64

    import cv2

    b64 = base64.b64encode(cv2.imencode(".jpg", room)[1].tobytes()).decode("ascii")
    res = client.post("/prepare", json={"image_b64": b64}).json()
    assert len(res["turns"]) == 4
    sizes = [cv2.imdecode(__import__("numpy").frombuffer(base64.b64decode(t), "uint8"), cv2.IMREAD_COLOR).shape[:2] for t in res["turns"]]
