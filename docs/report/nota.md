...
- *Output efficace.* Non richiede una valutazione aggiuntiva, ma si calcola incrociando i due parametri precedenti (realismo e qualità) espressi da ogni singolo utente. Una versione è considerata efficace se ottiene un voto di qualità di almeno 4 su 5 e, contemporaneamente, nessuna segnalazione di alterazione. La metrica risultante è il tasso di pubblicabilità: la percentuale di foto che l'agente immobiliare può mettere online direttamente, senza alcun bisogno di ritocco manuale.
...
**Come testare il prototipo.** Il prototipo funzionante è online su https://immobiliare-photo-lab.vercel.app e il codice è nel repository https://github.com/Pio57/immobiliare-photo-lab. Due modalità per provarlo: nella vista *Prova* si carica una foto scattata da sé (magari buia o un po' storta) e si vedono affiancati i tre flussi D1, D2 e D3, ciascuno con la sua scheda: cosa ha visto, quali moduli ha eseguito, tempo di elaborazione; nella vista *Studio* si scorrono le 24 foto precaricate rispondendo alle tre domande del test cieco, senza sapere quale workflow ha generato cosa.

**Architettura e strumenti.** Strumenti no-code e low-code: n8n self-hosted, dove vivono i quattro workflow (Correggi, Prodotto, Scelte, Batch) e da dove partono tutte le chiamate ai modelli; Claude Code, usato come strumento di supporto allo sviluppo del progetto. Modelli AI: Claude Haiku 4.5 (API Anthropic) per la diagnosi e la verifica del flusso D2; SDXL con ControlNet per il flusso generativo D3 e Real-ESRGAN per la super-risoluzione, entrambi tramite le API di Replicate. Codice: un servizio Python (FastAPI e OpenCV) per le correzioni, il controllo di fedeltà e i dati; il sito in React con Vite e Tailwind. Infrastruttura: Vercel per il sito, un VPS Hostinger con Docker per n8n e il servizio Python, GitHub per il codice e la storia del progetto.
...
**Limiti e prossimi passi.** Il limite principale è il campione: le 24 foto iniziali andranno ampliate e composte con più cura, definendo in anticipo quante foto per ciascun difetto (un certo numero troppo scure, un certo numero troppo chiare, un certo numero storte, e così via), in modo che ogni workflow sia messo alla prova su ogni difetto con lo stesso peso e i problemi di valutazione dei flussi si riducano a zero. I prossimi passi per far evolvere il prodotto sono due: portare la funzione in piattaforma per un test A/B, misurando l'impatto reale sui contatti generati; sviluppare un workflow di annuncio che non guardi più la singola foto ma l'intera galleria caricata dall'agente, per riordinare le immagini in base agli elementi che generano più interesse negli utenti e selezionare in automatico la foto con cui l'annuncio si apre.
